    ...
    if ((metadata.where() == metadata.cell) || (metadata.where() == metadata.node)) {
        ...
        if (N > 3) {
            throw std::invalid_argument(
                "_addArray() dims{} must have size in range [0,3]");
        }
        ...
        /*if (_pcellVars.find(label) == _pcellVars.end()) {
            ...
            _pcellVars.insert( std::pair<std::string, VariableVector<T>> (label,
        VariableVector<T>()) );
        }*/
        ...
        if (myMap.find(matIndex) != myMap.end()) {
            throw std::invalid_argument(
                "Duplicate material index in create MaterialVariable");
        }
...
void MaterialVariable<T>::DeleteMaterial(const int mat_id, const std::string label) {
    ...
    try {
        ...
        std::cout << "_______________________________________DELETING mat: " << label
                  << std::endl;
        vNew.~Variable();
        myMap.erase(mat_id);
