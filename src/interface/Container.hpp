...
class Container {
 public:
  ...
  Variable<T> &Get(const int index) { return *(s->_varArray[index]); }
  ...
  MaterialMap<T> &GetMaterial(const std::string &label) { return s->_matVars.Get(label); }
  ...
  std::vector<std::shared_ptr<Variable<T>>> &allVars() { return s->_varArray; }

  MaterialVariable<T> &matVars() { return s->_matVars; }
