...
class MaterialMap : public std::map<int, std::shared_ptr<Variable<T>>> {
 public:
  Variable<T> &operator()(int m) { return *(*this)[m]; }
  T &operator()(int m, int i) { return (*(*this)[m])(i); }
  T &operator()(int m, int j, int i) { return (*(*this)[m])(j, i); }
  T &operator()(int m, int k, int j, int i) { return (*(*this)[m])(k, j, i); }
  T &operator()(int m, int l, int k, int j, int i) { return (*(*this)[m])(l, k, j, i); }
...
class MaterialVariable {
 public:
  ...
  std::string info(const std::string &label) {
    ...
    for (auto const &items : myMap)
      s += std::to_string(items.first) + ":";
  ...
  MaterialMap<T> &Get(const std::string &label) {
    if (_cellVars.find(label) == _cellVars.end()) {
      throw std::invalid_argument("Unable to find material variable " + label +
  ...
  std::map<std::string, MaterialMap<T>> &getAllCellVars() { return _cellVars; }
  ...
  void print() {
    ...
    for (auto &m : _pcellVars) {
      std::cout << "    matvec: " << m.first << " has " << m.second.size() << " elements"
                << std::endl;
    }
  }
