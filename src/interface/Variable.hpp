...
class Variable : public AthenaArray<T> {
 public:
  ...
  Variable<T>(const std::string label, Metadata &metadata)
      : AthenaArray<T>(), _label(label), _m(metadata), mpiStatus(true) {
  ...
  Variable<T>(const std::string label, Variable<T> &src, const int dim, const int index,
    ...
    //    std::cout << "_____CREATED VAR SLICE: " << _label << ":" << this <<
    //    std::endl;
  }
...
struct FaceVariable : FaceField {
 public:
  ...
  FaceVariable(std::string label, FaceVariable &src)
      : FaceField(0, 0, 0, DATASTATUS::allocated), _label(label), _m(src.metadata()) {
...
struct EdgeVariable : EdgeField {
 public:
  ...
  EdgeVariable(const std::string label, const EdgeVariable &src)
      : EdgeField(0, 0, 0, DATASTATUS::allocated), _label(label), _m(src.metadata()) {
