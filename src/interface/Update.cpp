...
void FluxDivergence(Container<Real> &in, Container<Real> &dudt_cont) {
    ...
    Metadata m;
    ContainerIterator<Real> cin_iter(
        in, std::vector<parthenon::Metadata::flags>{m.independent});
    ContainerIterator<Real> cout_iter(
        dudt_cont, std::vector<parthenon::Metadata::flags>{m.independent});
...
void UpdateContainer(Container<Real> &in,
    ...
    ContainerIterator<Real> cin_iter(
        in, std::vector<parthenon::Metadata::flags>{m.independent});
    ContainerIterator<Real> cout_iter(
        out, std::vector<parthenon::Metadata::flags>{m.independent});
    ContainerIterator<Real> du_iter(
        dudt_cont, std::vector<parthenon::Metadata::flags>{m.independent});
    int nvars = cout_iter.vars.size();
...
void AverageContainers(Container<Real> &c1, Container<Real> &c2, const Real wgt1) {
    ...
    Metadata m;
    ContainerIterator<Real> c1_iter(
        c1, std::vector<parthenon::Metadata::flags>{m.independent});
    ContainerIterator<Real> c2_iter(
        c2, std::vector<parthenon::Metadata::flags>{m.independent});
