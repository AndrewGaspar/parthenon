...
void Container<T>::Add(const std::string label, const Metadata &metadata,
  ...
  if (metadata.hasMaterials()) {
    ...
    //                      pmy_block->ncells3, pmy_block->ncells2,
    //                      pmy_block->ncells1));
    return;
...
void Container<T>::SendBoundaryBuffers() {
  ...
  for (auto &v : s->_varArray) {
    if ((v->metadata()).fillsGhost()) {
      if (!v->mpiStatus) {
        std::cout << "        sending without the receive, something's up:" << v->label()
                  << std::endl;
      }
  ...
  for (auto &myMap : s->_matVars.getAllCellVars()) {
    ...
    for (auto &v : myMap.second) {
      ...
      if (!v.second->mpiStatus) {
        std::cout << "        _________Err:" << v.first
                  << ":sending without the receive, something's up:" << v.second->label()
                  << std::endl;
      }
  ...
  for (auto label : names) {
    ...
    try { // normal variable
      ...
      try { // material variable
        ...
        throw std::invalid_argument(" Unable to find variable " + label +
                                    " in container");
      } // material variable
    }   // normal variable
