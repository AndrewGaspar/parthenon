...
void VTKOutput::WriteContainer(Mesh *pm, ParameterInput *pin, bool flag) {
    ...
    while (pmb != nullptr) {
        ...
        //  5. Data.  An arbitrary number of scalars and vectors can be written (every
        //  node in the OutputData doubly linked lists), all in binary floats format
        ...
        for (auto &v : ci.vars) {
            ...
            for (int k = out_ks; k <= out_ke; k++) {
                for (int j = out_js; j <= out_je; j++) {
                    ...
                    std::fwrite(
                        data, sizeof(float), static_cast<std::size_t>(ncells1), pfile);
                }
            }
        }
