...
void FormattedTableOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool flag) {
    ...
    while (pmb != nullptr) {
        ...
        // write data col headers from "name" stored in OutputData nodes of doubly linked
        // list
        ...
        for (int k = out_ks; k <= out_ke; ++k) {
            for (int j = out_js; j <= out_je; ++j) {
                for (int i = out_is; i <= out_ie; ++i) {
                    ...
                    if (out_js != out_je) {
                        std::fprintf(
                            pfile, " %04d", j); // note extra space for formatting
                        std::fprintf(pfile,
                                     output_params.data_format.c_str(),
                                     pmb->pcoord->x2v(j));
                    }
                    if (out_ks != out_ke) {
                        std::fprintf(
                            pfile, " %04d", k); // note extra space for formatting
                        std::fprintf(pfile,
                                     output_params.data_format.c_str(),
                                     pmb->pcoord->x3v(k));
                    }
