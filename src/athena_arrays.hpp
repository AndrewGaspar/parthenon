...
class AthenaArray {
  public:
    ...
    AthenaArray(
        int nx4, int nx3, int nx2, int nx1, DataStatus init = DataStatus::allocated)
        : pdata_(nullptr),
          nx1_(nx1),
          nx2_(nx2),
          nx3_(nx3),
    ...
    // array.NewAthenaArray() or array.InitWithShallowSlice() (only used in outputs.cpp +
    // 3x other files)
    // TODO(felker): replace InitWithShallowSlice with ??? and remove shallow_copy enum
    // val
    // TODO(felker): replace raw pointer with std::vector + reshape (if performance is
    // same)
    ...
    NewAthenaArray(int nx5, int nx4, int nx3, int nx2, int nx1);
    __attribute__((nothrow)) void
    NewAthenaArray(int nx6, int nx5, int nx4, int nx3, int nx2, int nx1);
    ...
    std::vector<int> GetShape() const {
        return std::vector<int>({nx6_, nx5_, nx4_, nx3_, nx2_, nx1_});
    }
    ...
    // TODO(felker): Rename function. Conflicts with "AthenaArray<> data" OutputData
    // member.
    ...
    // int l?, int o?
    T &operator()(
        const int p, const int m, const int n, const int k, const int j, const int i) {
    ...
    AthenaArray<T> operator*(T scale) const {
        std::transform(
            pdata_, pdata_ + GetSize(), pdata_, [scale](T val) { return scale * val; });
    }
    ...
    friend AthenaArray<T> operator+(const AthenaArray<T> &lhs,
                                    const AthenaArray<T> &rhs) {
    ...
    void InitWithShallowSlice(const AthenaArray<T> &src,
                              const int dim,
                              const int indx,
                              const int nvar);
...
template <typename T>
AthenaArray<T> operator*(const T scale, const AthenaArray<T> &arr) {
    return arr * scale;
}
...
__attribute__((nothrow)) AthenaArray<T>::AthenaArray(const AthenaArray<T> &src) {
    ...
    if (src.pdata_) {
        std::size_t size =
            (src.nx1_) * (src.nx2_) * (src.nx3_) * (src.nx4_) * (src.nx5_) * (src.nx6_);
...
template <typename T>
__attribute__((nothrow)) AthenaArray<T> &AthenaArray<T>::
operator=(const AthenaArray<T> &src) {
    if (this != &src) {
        // setting nxN_ is redundant given the above (unenforced) constraint on allowed
        // usage
        nx1_ = src.nx1_;
        nx2_ = src.nx2_;
        nx3_ = src.nx3_;
...
__attribute__((nothrow)) AthenaArray<T>::AthenaArray(AthenaArray<T> &&src) {
    ...
    nx6_ = src.nx6_;
    if (src.pdata_) {
        // && (src.state_ != DataStatus::allocated){  // (if forbidden to move shallow
        // slices)
        //  ---- >state_ = DataStatus::allocated;
...
template <typename T>
__attribute__((nothrow)) AthenaArray<T> &AthenaArray<T>::operator=(AthenaArray<T> &&src) {
    if (this != &src) {
...
bool AthenaArray<T>::operator==(const AthenaArray<T> &rhs) const {
    return (pdata_ == rhs.pdata_ && state_ == rhs.state_ && nx1_ == rhs.nx1_ &&
            nx2_ == rhs.nx2_ && nx3_ == rhs.nx3_ && nx4_ == rhs.nx4_ &&
            nx5_ == rhs.nx5_ && nx6_ == rhs.nx6_);
}
...
template <typename T>
void AthenaArray<T>::InitWithShallowSlice(const AthenaArray<T> &src,
                                          const int dim,
                                          const int indx,
                                          const int nvar) {
    pdata_ = src.pdata_;
...
template <typename T>
__attribute__((nothrow)) void
AthenaArray<T>::NewAthenaArray(int nx4, int nx3, int nx2, int nx1) {
    state_ = DataStatus::allocated;
    nx1_ = nx1;
    nx2_ = nx2;
    nx3_ = nx3;
...
AthenaArray<T>::NewAthenaArray(int nx5, int nx4, int nx3, int nx2, int nx1) {
    ...
    nx6_ = 1;
    pdata_ =
        new T[nx1 * nx2 * nx3 * nx4 * nx5](); // allocate memory and initialize to zero
}
...
template <typename T>
__attribute__((nothrow)) void
AthenaArray<T>::NewAthenaArray(int nx6, int nx5, int nx4, int nx3, int nx2, int nx1) {
    state_ = DataStatus::allocated;
    nx1_ = nx1;
    nx2_ = nx2;
    nx3_ = nx3;
