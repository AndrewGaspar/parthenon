...
#pragma omp simd simdlen(SIMD_WIDTH)
    ...
    // Cartesian-like coordinate with nonuniform mesh spacing
  } else {
    for (int n = 0; n < NWAVE; ++n) {
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
...
#pragma omp simd simdlen(SIMD_WIDTH)
    for (int i = il; i <= iu; ++i) {
      wl(n, i + 1) =
          wc(n, i) + ((pco->x1f(i + 1) - pco->x1v(i)) / pco->dx1f(i)) * dwm(n, i);
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        // dq2 > 0 ---> dqF, dqB are nonzero and have the same sign ----> no risk
        // for (dqF + dqB) = 0 cancellation causing a divide-by-0 in the above
        // line
