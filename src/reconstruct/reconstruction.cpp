...
Reconstruction::Reconstruction(MeshBlock *pmb, ParameterInput *pin)
    ...
    // limiter formulations for Schwarzschild, Kerr metrics instead of Cartesian-like
    // wghts
    ...
    if ((xorder == 3) || (xorder == 4)) {
        ...
        // coorddinate reconstruction weights. Same data structures are reused at each
        // spatial index (i or j) and for both PPMx1 and PPMx2 weight calculations:
...
#pragma omp simd
            for (int i = (pmb->is) - NGHOST + 1; i <= (pmb->ie) + NGHOST - 1; ++i) {
                ...
                if (i >
                    (pmb->is) - NGHOST + 1) { // c3-c6 are not computed in first iteration
