...
#pragma omp simd
            for (int i = il; i <= iu; ++i) {
                // KGF: add the off-centered quantities first to preserve FP symmetry
                d2qf(i) =
                    6.0 * (dph(i) + dph_ip1(i) - 2.0 * q_i(n, i)); // a6 coefficient * -2
            }
...
#pragma omp simd
            for (int i = il; i <= iu; ++i) {
                // KGF: add the off-centered quantities first to preserve FP symmetry
                d2qf(i) =
                    6.0 * (dph(i) + dph_jp1(i) - 2.0 * q_j(n, i)); // a6 coefficient * -2
            }
...
void Reconstruction::PiecewiseParabolicX3(const int k,
    ...
    AthenaArray<Real> &d2qc_km1 = scr07_i_, &d2qc = scr08_i_, &d2qc_kp1 = scr09_i_,
                      &d2qf = scr10_i_;
...
#pragma omp simd
            for (int i = il; i <= iu; ++i) {
                // KGF: add the off-centered quantities first to preserve FP symmetry
                d2qf(i) =
                    6.0 * (dph(i) + dph_kp1(i) - 2.0 * q_k(n, i)); // a6 coefficient * -2
            }
            //--- Step 2b.
            //---------------------------------------------------------------------
