...
MeshBlock::MeshBlock(int igid,
  ...
  // terms, and may enroll quantities in AMR and BoundaryVariable objs. in
  // BoundaryValues
  ...
  // in order to correctly advance the BoundaryValues::bvars_next_phys_id_ local
  // counter.

  // TODO(felker): check that local counter pbval->bvars_next_phys_id_ agrees with
  // shared Mesh::next_phys_id_ counter (including non-BoundaryVariable / per-MeshBlock
  // reserved values). Compare both private member variables via
...
void MeshBlock::AllocateRealUserMeshBlockDataField(int n) {
  if (nreal_user_meshblock_data_ != 0) {
    ...
    msg << "### FATAL ERROR in MeshBlock::AllocateRealUserMeshBlockDataField" << std::endl
...
void MeshBlock::AllocateIntUserMeshBlockDataField(int n) {
  if (nint_user_meshblock_data_ != 0) {
    std::stringstream msg;
    msg << "### FATAL ERROR in MeshBlock::AllocateIntusermeshblockDataField" << std::endl
        << "User MeshBlock data arrays are already allocated" << std::endl;
    ATHENA_ERROR(msg);
    return;
  }
...
void MeshBlock::AllocateUserOutputVariables(int n) {
  ...
  if (nuser_out_var != 0) {
    std::stringstream msg;
    msg << "### FATAL ERROR in MeshBlock::AllocateUserOutputVariables" << std::endl
...
void MeshBlock::SetUserOutputVariableName(int n, const char *name) {
  if (n >= nuser_out_var) {
    std::stringstream msg;
    msg << "### FATAL ERROR in MeshBlock::SetUserOutputVariableName" << std::endl
