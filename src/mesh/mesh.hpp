...
class MeshBlockApplicationData {
  public:
    // make this pure virtual so that this class cannot be instantiated (only derived
...
class MeshBlock {
  ...
  public:
    ...
    // "standard" sized MeshBlock arrays, depending on ndim (i.e. ncells2=nx2+2*NGHOST if
    // nx2>1)
    ...
    // t^{n+1}, the time-integration scheme may partially substep several storage register
    // pairs (u,b), (u1,b1), (u2, b2), ..., (um, bm) through the dt interval. Track their
    // time abscissae at the end of each stage (1<=l<=nstage) as (dt_m^l) relative to t^n
...
class Mesh {
  ...
  public:
    ...
    // and other categories of MPI communication for generating unique MPI_TAGs
  ...
  private:
    // data
    int next_phys_id_; // next unused value for encoding final component of MPI tag
                       // bitfield
    ...
    // (unlike LogicalLocation.lxi, nrbxi don't grow w/ AMR # of levels, so keep 32-bit
    // int)
    int nrbx1, nrbx2, nrbx3;
    // TODO(felker) find unnecessary static_cast<> ops. from old std::int64_t type in
    // 2018:
    ...
    void
    FillSameRankFineToCoarseAMR(MeshBlock *pob, MeshBlock *pmb, LogicalLocation &loc);
    void
    ...
    // DEPRECATED(felker): provide trivial overload for old-style BoundaryFace enum
    // argument
...
ComputeMeshGeneratorX(std::int64_t index, std::int64_t nrange, bool sym_interval) {
    ...
    if (!sym_interval) {
        ...
        // conversion if nrange is even, there is an index at center x=0.0; map it to
        // (int) 0 if nrange is odd, the center x=0.0 is between two indices; map them to
        // -1, 1
