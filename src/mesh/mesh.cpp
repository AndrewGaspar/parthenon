...
Mesh::Mesh(ParameterInput *pin,
    ...
    if (block_size.nx1 < 4 || (block_size.nx2 < 4 && f2) || (block_size.nx3 < 4 && f3)) {
        msg << "### FATAL ERROR in Mesh constructor" << std::endl
    ...
    if (multilevel) {
        if (block_size.nx1 % 2 == 1 || (block_size.nx2 % 2 == 1 && f2) ||
            ...
            msg << "### FATAL ERROR in Mesh constructor" << std::endl
                << "The size of MeshBlock must be divisible by 2 in order to use SMR or "
                   "AMR."
                << std::endl;
            ATHENA_ERROR(msg);
        }
                ...
                if (ndim == 3) {
                    for (std::int64_t k = lx3min; k < lx3max; k += 2) {
                        for (std::int64_t j = lx2min; j < lx2max; j += 2) {
                            for (std::int64_t i = lx1min; i < lx1max; i += 2) {
                                LogicalLocation nloc;
                                nloc.level = lrlev, nloc.lx1 = i, nloc.lx2 = j,
                                nloc.lx3 = k;
                                int nnew;
                                tree.AddMeshBlock(nloc, nnew);
                            }
...
#pragma omp for
            for (int i = 0; i < nmb; ++i) {
                pmb_array[i]->real_container.StartReceiving(
...
#pragma omp parallel for num_threads(nthreads)
    for (int i = 0; i < nmb; ++i) {
        pmb_array[i]->SetBlockTimestep(
            Update::EstimateTimestep(pmb_array[i]->real_container));
    }
...
void Mesh::SetBlockSizeAndBoundaries(LogicalLocation loc,
    ...
    if (mesh_size.nx2 == 1) {
        ...
        if (lx2 == (nrbx_ll)-1) {
            ...
            Real rx =
                ComputeMeshGeneratorX(lx2 + 1, nrbx_ll, use_uniform_meshgen_fn_[X2DIR]);
    ...
    if (mesh_size.nx3 == 1) {
        ...
        if (lx3 == (nrbx_ll)-1) {
            ...
            Real rx =
                ComputeMeshGeneratorX(lx3 + 1, nrbx_ll, use_uniform_meshgen_fn_[X3DIR]);
                ...
                if (dt_diagnostics != -1) {
                    ...
                    if (UserTimeStep_ != nullptr) {
                        ...
                        std::cout << "\ndt_user=" << dt_user
                                  ...
                                  << ratio << std::setprecision(dt_precision);
                    }
