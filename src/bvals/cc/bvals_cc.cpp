...
int CellCenteredBoundaryVariable::LoadBoundaryBufferToCoarser(Real *buf,
    ...
    pmb->pmr->RestrictCellCenteredValues(
        var, coarse_var, nl_, nu_, si, ei, sj, ej, sk, ek);
...
void CellCenteredBoundaryVariable::SetBoundaryFromCoarser(Real *buf,
    ...
    // pmb->pmr->ProlongateCellCenteredValues(coarse_var, *var_cc, nl_, nu_, si, ei, sj,
    // ej, sk, ek);
}
        ...
        if (nb.snb.rank != Globals::my_rank) {
            ...
            if (pmy_mesh_->multilevel && nb.ni.type == NeighborConnect::face) {
                ...
                if (nb.fid == 0 || nb.fid == 1)
                    size =
                        ((pmb->block_size.nx2 + 1) / 2) * ((pmb->block_size.nx3 + 1) / 2);
                else if (nb.fid == 2 || nb.fid == 3)
                    size =
                        ((pmb->block_size.nx1 + 1) / 2) * ((pmb->block_size.nx3 + 1) / 2);
                else // (nb.fid == 4 || nb.fid == 5)
                    size =
                        ((pmb->block_size.nx1 + 1) / 2) * ((pmb->block_size.nx2 + 1) / 2);
                size *= (nu_ + 1);
                if (nb.snb.level < mylevel) { // send to coarser
                    tag = pmb->pbval->CreateBvalsMPITag(
                        nb.snb.lid, nb.targetid, cc_flx_phys_id_);
                    ...
                    tag = pmb->pbval->CreateBvalsMPITag(
                        pmb->lid, nb.bufid, cc_flx_phys_id_);
