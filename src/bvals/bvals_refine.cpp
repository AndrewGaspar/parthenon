...
void BoundaryValues::ProlongateBoundaries(const Real time, const Real dt) {
    ...
    // surrounding the boundary zone to calculate the slopes ("ghost-ghost zone"). 3x
    // steps:
    for (int n = 0; n < nneighbor; n++) {
        ...
        // Step 1. Apply necessary variable restrictions when ghost-ghost zone is on same
        // lvl
...
void BoundaryValues::RestrictGhostCellsOnSameLevel(const NeighborBlock &nb,
    ...
    for (auto fc_pair : pmr->pvars_fc_) {
        ...
        if (pmb->block_size.nx2 > 1) {
            ...
            pmr->RestrictFieldX2(
                (*var_fc).x2f, (*coarse_fc).x2f, ris, rie, rs, re, rks, rke);
        } else { // 1D
            pmr->RestrictFieldX2(
                (*var_fc).x2f, (*coarse_fc).x2f, ris, rie, rjs, rje, rks, rke);
        ...
        if (pmb->block_size.nx3 > 1) {
            ...
            pmr->RestrictFieldX3(
                (*var_fc).x3f, (*coarse_fc).x3f, ris, rie, rjs, rje, rs, re);
        } else { // 1D or 2D
            pmr->RestrictFieldX3(
                (*var_fc).x3f, (*coarse_fc).x3f, ris, rie, rjs, rje, rks, rke);
...
void BoundaryValues::ProlongateGhostCells(
    ...
    for (auto cc_pair : pmr->pvars_cc_) {
        ...
        pmr->ProlongateCellCenteredValues(
            *coarse_cc, *var_cc, 0, nu, si, ei, sj, ej, sk, ek);
    }
