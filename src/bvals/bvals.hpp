...
class BoundaryBase {
  ...
  private:
    // calculate 3x shared static data members when constructing only the 1st class
    // instance int maxneighbor_=BufferID() computes ni[] and then calls
...
class BoundaryValues : public BoundaryBase, // public BoundaryPhysics,
  ...
  private:
    ...
    // C++11: direct-list-initialization -> value init of array -> zero init of each
    // scalar
    ...
    // ProlongateBoundaries() wraps the following S/AMR-operations (within nneighbor
    // loop): (the next function is also called within 3x nested loops over nk,nj,ni)
    void RestrictGhostCellsOnSameLevel(const NeighborBlock &nb, int nk, int nj, int ni);
    void ApplyPhysicalBoundariesOnCoarseLevel(const NeighborBlock &nb,
                                              const Real time,
                                              const Real dt,
                                              int si,
                                              int ei,
